...
spatial_cell_name(model::ModelSettings) = model.geometry == :trellis ? model.trellis_cell : model.spatial_cell
spatial_ladders(model::ModelSettings) = spatial_cell_name(model) == :one_ladder ? 1 : 2
_spatial_state_kind(model::ModelSettings) = model.geometry == :trellis ? "trellis_mps_mft_state" : "spatial_cell_mps_mft_state"
_spatial_seed_kind(model::ModelSettings) = model.geometry == :trellis ? "trellis_correlation_seed" : "spatial_cell_correlation_seed"

"""Square A/B: each leg receives the other ladder's opposite leg at the same rung.
The two physical neighbors have the same B (or A) representative. They are
separate interfaces; no extra coordination factor or longitudinal shift is added.
"""
function spatial_cell_mean_fields(correlations::AbstractVector{CorrelationState}, model::ModelSettings)
    model.geometry == :trellis && return trellis_mean_fields(correlations, model)
    model.geometry == :square && model.spatial_cell == :two_ladder ||
        throw(ArgumentError("expected square two-ladder spatial cell"))
    length(correlations) == 2 || throw(DimensionMismatch("square A/B needs two correlation states"))
    return [mean_fields_from_correlations(correlations[2], model),
            mean_fields_from_correlations(correlations[1], model)]
end
...
function _trellis_initial_states(settings::ProjectSettings)
    ...
    count = spatial_ladders(model)
    if settings.run.resume_checkpoint !== nothing
        verify_resume!(settings)
        return h5open(settings.run.resume_checkpoint, "r") do file
            read(file, "artifact_kind") == _spatial_state_kind(model) || error("expected a complete spatial-cell checkpoint")
            read(file, "provenance/model_fingerprint") == model_fingerprint(model) || error("spatial resume model mismatch")
            read(file, "spatial_ladders") == count || error("spatial resume cell mismatch")
    ...
    settings.run.inherit_from !== nothing || throw(ArgumentError("spatial-cell starts require a correlation-template seed"))
    verify_inherit!(settings)
    fields = h5open(settings.run.inherit_from, "r") do file
        read(file, "artifact_kind") == _spatial_seed_kind(model) || error("wrong spatial-cell seed kind")
        read(file, "model_fingerprint") == model_fingerprint(model) || error("spatial seed model mismatch")
        templates = if haskey(file, "template_correlations")
            fill(_trellis_read_correlations(file["template_correlations"]), count)
        else
            [_trellis_read_correlations(file["ladders/$(i == 1 ? "A" : "B")/template_correlations"]) for i in 1:count]
        end
        generated = spatial_cell_mean_fields(templates, model)
        for index in 1:count
            ...
            all(getfield(stored, key) == getfield(generated[index], key) for key in fieldnames(FieldState)) ||
                error("spatial seed kernel/readback mismatch")
        end
        generated
    end
    return map(1:count) do index
        sites = make_sites(model, settings.runtime)
        # Same product-state configuration; distinct template fields can seed
        # A/B asymmetry without confounding it with a different random start.
...
function _write_trellis_checkpoint(path; settings, psis, histories, diagnostic, members,
    ...
    h5open(temporary, "w") do file
        _write_dict(file, Dict(
            "schema_version" => 1, "artifact_kind" => _spatial_state_kind(settings.model),
        ...
        file["model/transverse_geometry"] = String(settings.model.geometry)
        ...
        for index in 1:count
            ...
            group["longitudinal_origin"] = settings.model.geometry == :trellis && index == 2 ? -0.5 : 0.0
...
function run_spatial_cell_scf(settings::ProjectSettings)
    ...
    prefix = settings.model.geometry == :trellis ? "trellis" : "spatial_cell"
    provenance["$(prefix)_update"] = "simultaneous raw spatial-cell map; fixed coordinates"
    provenance["$(prefix)_density_constraint"] = "target density on each ladder separately"
    provenance["$(prefix)_energy"] = "current simultaneous cell correlations; centered normal functional"
    ...
    for iteration in 1:settings.run.max_iterations
        ...
        measured = spatial_cell_mean_fields([r.correlations for r in results], settings.model)
        for index in 1:count
            r = results[index]
        ...
        for index in 1:count
            print("$(prefix)_ladder=$(index == 1 ? "A" : "B") ")
            _print_iteration(last(histories[index]), members[index], results[index].result)
        end
        @printf("%s cell_sweep=%d ladders=%d Evar_target/site=%.12f status=%s\n", uppercase(prefix), iteration, count,
    ...
    open(summary_path, "w") do io
        println(io, "# $(settings.model.geometry) spatial-cell result\n")
        println(io, "Cell: $(spatial_cell_name(settings.model)); $(count) ladder(s), $(length(first(histories))) simultaneous raw sweeps.")
...
# Existing entry point retained for callers of the original trellis driver.
run_trellis_scf(settings::ProjectSettings) = run_spatial_cell_scf(settings)
