function _result_row(path::AbstractString)
    return h5open(path, "r") do file
        ...
        energy = haskey(file, "energy/canonical_variational_energy") ? Float64(read(file, "energy/canonical_variational_energy")) : NaN
        branch = haskey(file, "provenance/branch_label") ? String(read(file, "provenance/branch_label")) : "unknown"
        return (; path=abspath(path), completed, accepted, status, geometry, fingerprint,
...
function select_completed_runs(directory::AbstractString; include_incomplete::Bool=false, geometry=nothing)
    ...
    for path in sort(paths)
        ...
        !include_incomplete && !(row.completed && row.accepted && row.status == "fixed_point") && continue
        push!(rows, merge(row, (; plot_style=row.accepted ? "solid" : "hatched")))
    end
    return rows
end
...
function compare_variational_branches(paths::AbstractVector{<:AbstractString})
    ...
    all(row -> row.completed && row.accepted && row.status == "fixed_point", rows) ||
        throw(ArgumentError("only accepted fixed-point branches may enter the variational ranking"))
