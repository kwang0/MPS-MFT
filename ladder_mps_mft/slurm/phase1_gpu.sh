...
readonly PHASE1_SCRIPT_VERSION="1.0.0"
...
load_environment() {
  ...
  [[ "${PHASE1_RUN_SCRIPT_VERSION:-missing}" == "$PHASE1_SCRIPT_VERSION" ]] || die \
    "run script version ${PHASE1_RUN_SCRIPT_VERSION:-missing} differs from $PHASE1_SCRIPT_VERSION"
...
case "$action" in
  ...
  submit-matrix)
    ...
    run_dir="$(resolve_run_dir "$2")"; load_environment "$run_dir"; submit_matrix_jobs "$run_dir"
    ;;
  continue)
    [[ $# == 3 ]] || die "continue requires RUN_ID LABEL"
    require_command sbatch; require_command sacct
    run_dir="$(resolve_run_dir "$2")"; load_environment "$run_dir"; continue_branch "$run_dir" "$3"
    ;;
  submit-ep)
