...
load_environment() {
  ...
  [[ "${PHASE0_RUN_SCRIPT_VERSION:-}" == "$PHASE0_SCRIPT_VERSION" ]] || die \
    "run.env script version ${PHASE0_RUN_SCRIPT_VERSION:-missing} differs from worker version $PHASE0_SCRIPT_VERSION"
...
submit_matrix_jobs() {
  ...
  while IFS=$'\t' read -r label threads backend logical; do
    raw="$(sbatch --parsable --account="$PHASE0_ACCOUNT" --constraint=cpu --qos="$PHASE0_QOS" \
      ...
      --dependency="afterok:$seed_id" --kill-on-invalid-dep=yes \
