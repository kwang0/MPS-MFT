...
println("accepted,status,style,geometry,branch,canonical_variational_energy,path")
for row in rows
    @printf("%s,%s,%s,%s,%s,%.16g,%s\n", row.accepted, row.status, row.plot_style, row.geometry, row.branch, row.energy, row.path)
end
