...
def main(trellis=False):
    out = PROJECT / "output/source_bundles/trellis_comparison_20260916.zip" if trellis else OUT
    ...
    if trellis:
        for name in ("README.md", "prepared_branches.csv"):
            relative = "docs/reports/trellis_comparison_20260916/" + name
            files[relative] = PROJECT / relative
        files["docs/TRELLIS_MEAN_FIELD.md"] = PROJECT / "docs/TRELLIS_MEAN_FIELD.md"
        files["data/two_basin_references.h5"] = PROJECT / "data/two_basin_references.h5"
    else:
    ...
    with zipfile.ZipFile(staging, "w", compression=zipfile.ZIP_DEFLATED) as z:
        for name, data in contents.items():
            info = zipfile.ZipInfo(name, date_time=(2026, 9, 16 if trellis else 8, 0, 0, 0))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--trellis", action="store_true", help="package the four-run trellis campaign")
    main(trellis=parser.parse_args().trellis)
