# Plan: record the bare-survey and isolated-ladder-backbone program

## Context

The review of `ladder_mps_mft` (earlier in this session) found that the mean-field (MF)
convergence problems since v3 are a classification and reference-point problem: the
"period-two candidates" are monotone drift, and seeds added to zero field are not
perturbations of any fixed point. The discussion converged on a two-tier program: a cheap
geometry-independent bare survey of the isolated ladder's linear response, done together
with a proper isolated-ladder backbone (E_p, gaps, K_rho, correlators, saved states), and a
later dressed tier only where needed. The user asked for this discussion and the outline to
be recorded in a new markdown file for reference. Nothing else changes.

## Change

Create one new file, `ladder_mps_mft/docs/BARE_SURVEY_AND_LADDER_BACKBONE.md`, with the
content below. No source, config, ledger, registry, or artifact is modified. Optionally add
one line to `ladder_mps_mft/README.md` pointing at the new doc (ask before doing so; not
required).
...
point, all CPU QN DMRG; measure one point at chi 400 before budgeting the 3x3 square
grid and the t0 = 1.0:0.025:1.2 frustrated line. Probes are short because psi_0 is a
good start; the sector solves dominate.
...
- The file renders and its tables are well formed (view in the editor).
- `julia --project=. -e 'using Pkg; Pkg.test()'` is unaffected (docs only); the
  implementation fingerprint is unaffected because `.md` files are not hashed.
- `git status` shows only the new doc (and README line if added).
